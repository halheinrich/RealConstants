...
/// <param name="CadenceTest">
/// The test that measures <paramref name="Cadence"/>, as <c>Class.Method</c> in
/// <c>RealConstants.Tests</c>. <c>../AGENTS.md</c> section Reliability clause (f) asks a
/// quantitative claim to carry its basis, and a named test is a basis a reader can run:
/// <c>dotnet test --filter FullyQualifiedName~&lt;name&gt;</c> takes it as printed. It is a field
/// rather than a phrase inside the figure so that it can be checked - <c>CatalogueTests</c>
/// asserts that each one names a test that exists, in the provider's own test class - and a
/// string rather than a <c>nameof</c> because this project does not reference the tests.
/// </param>
